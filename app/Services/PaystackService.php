...
class PaystackService implements PaymentInterface
{
    ...
    public function verifyPayment($reference,$user_id)
    {
        ...
        elseif($get_bal->balance == 0.00 && $amount >  $min){
           ...
           if($amount >= 5000){
               $check_stats = DB::table('user_statistics')->where('user_id',auth()->user()->id)->first();
                if(is_null($check_stats->wallet_topup)){
                    DB::table('user_statistics')
                        ->where('user_id', auth()->user()->id)
                        ->update(['wallet_topup'=>TopCoinAmount::TopCoin]);
                }else{
                    DB::table('user_statistics')
                        ->where('user_id', auth()->user()->id)
                        ->increment('wallet_topup',TopCoinAmount::TopCoin);
                }
           }  
        ...
        //    elseif($amount < $get_bal->minimium_balance){
        ...
        $r->user_id = auth()->id();
        $r->funds_added_count = AddFundToWallet::AddFund;
        $r->save();

        }else{
        $rfind = DB::table('user_statistics')->where('user_id',$exist_user_count->user_id)->increment('funds_added_count',AddFundToWallet::AddFund);
        }
