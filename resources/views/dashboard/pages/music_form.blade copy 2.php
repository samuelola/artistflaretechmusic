...
<main class="dashboard-main">
  ...
  <div id="queueOverlay" 
     style="display:none; position:fixed; top:0; left:0; width:100%; height:100%;
            background:rgba(0,0,0,0.6); z-index:9999; color:#fff; text-align:center; font-size:20px;">
...
$(function(){
  ...
  function showQueueOverlay() {
    $('#queueOverlay').fadeIn(200);
  }

  function hideQueueOverlay() {
    $('#queueOverlay').fadeOut(200);
  }
 ...
 $('#goto2').on('click', function() {
  ...
  $('#uploadArtworkBtn').on('click', function(){
    ...
    $.ajax({
      ...
      error(){ $('#artworkStatus').text('Error uploading artwork'); }
    });
  });
...
$('#audioList').on('click', '.remove-audio', async function() {
  ...
  const filename = item.find('strong').text().trim();
  ...
  // Find file meta
  const fileMeta = uploadedFilesMeta.find(f => f.filename === filename);

  // If track exists on backend, delete it too
  if (fileMeta?.track_id && musicReleaseId) {
    try {
  ...
  // Remove matching track card (by track_id or filename)
  if (fileMeta?.track_id) {
    $(`#tracksContainer .track-card[data-track-id="${fileMeta.track_id}"]`).remove();
  } else {
    $(`#tracksContainer .track-card:contains("${filename}")`).remove();
  }

  // Remove from uploadedFilesMeta array
  uploadedFilesMeta = uploadedFilesMeta.filter(f => f.filename !== filename);

  // If no audios remain, reset input and UI
...
$('#audioList').on('click', '#clearAllAudios', async function() {
  ...
  if (!releaseId) { alert('Save Step 1 first'); return; }
  if (!uploadedFilesMeta.length) { alert('Select audio files first and wait for durations to be extracted.'); return; }
  ...
  $('#audioUploadStatus').html(`
    ...
    <div class="small text-muted mt-1">Uploading... please wait</div>
  `);

  showQueueOverlay(); // <--- show overlay immediately
  ...
  $.ajax({
    ...
    success(resp) {
      ...
      if (resp.status === 'processing') {
        $('#audioUploadStatus').append('<div class="text-muted mt-2">Processing audio files... please wait</div>');
        pollAudioUploadStatus(resp.cache_key, (files, releaseId) => {
        console.log('Upload complete:', files);
        $('#music_release_id').val(releaseId);
        buildTrackForms(files); // Your existing function
        // after 1 minute, check again just in case
        setTimeout(() => {
          $.get(`/audio_upload/status/${resp.cache_key}`, res => {
            if (res.status === 'ok') buildTrackForms(res.files);
          });
        }, 60000); 
      });
    }
  ...
  files.forEach((f, index) => {
        ...
        <div class="card-body">
          ...
          <div class="row mb-3">
            <!--<div class="col-md-4">
              ...
              <input class="form-control track-number" type="number" value="${index+1}">
            </div>-->
            <div class="col-md-4">
          ...
          <div class="participants-section">
            ...
            <button class="btn btn-sm btn-outline-primary-600 add-participant">Add Participant</button>
          </div>
        </div>
      </div>
    `);
    ...
    card.find('.participants-list').append(buildParticipantRowHtml());
...
$(document).on('click', '.add-note-btn', function () {
 ...
 $('#tracksContainer').on('click', '.remove-row', function () {
  ...
  $('#tracksContainer .track-card').each(function (index) {
    if (hasError) return false; // break outer loop if already error found
    ...
    if (!title) {
      hasError = true;
      errorMsg = `Track ${trackIndex}: Title is required.`;
      return false;
    }
    if (!artist) {
      hasError = true;
      errorMsg = `Track ${trackIndex}: Artist is required.`;
      return false;
    }
    if (!feature_artist) {
      hasError = true;
      errorMsg = `Track ${trackIndex}: Featured Artist is required.`;
      return false;
    }
    if (!instrumental) {
      hasError = true;
      errorMsg = `Track ${trackIndex}: Instrumental is required.`;
      return false;
    }
    if (!language) {
      hasError = true;
      errorMsg = `Track ${trackIndex}: Language is required.`;
      return false;
    }
    if (!parental) {
      hasError = true;
      errorMsg = `Track ${trackIndex}: Parental field is required.`;
      return false;
    }
    if (!Array.isArray(genre) || genre.length === 0) {
      hasError = true;
      errorMsg = `Track ${trackIndex}: At least one Genre is required.`;
      return false;
    }
    if (!Array.isArray(stream_type) || stream_type.length === 0) {
      hasError = true;
      errorMsg = `Track ${trackIndex}: Stream type (For) is required.`;
      return false;
    }

    // Participant validation
    const participants = [];
    const $participantRows = $card.find('.participant-row');
    ...
    $participantRows.each(function (pIdx) {
      ...
      if (!participantName) {
        ...
        hasError = true;
        errorMsg = `Track ${trackIndex}, Participant ${pIndex}: Participant name is required.`;
        return false;
      }
      ...
      if (isNaN(payout) || payout < 0) {
        participantError = true;
        hasError = true;
        errorMsg = `Track ${trackIndex}, Participant ${pIndex}: Payout must be a valid non-negative number.`;
        return false;
      }
    ...
    // Round total to 2 decimals to avoid floating-point precision issues
    totalPayout = Math.round(totalPayout * 100) / 100;

    // Validate total payout === 100
    if (totalPayout !== 100) {
      hasError = true;
      errorMsg = `Track ${trackIndex}: Total participant payout must equal exactly 100%. (Currently: ${totalPayout}%)`;
      return false;
    }
  ...
  // If validation failed, show alert and stop
  if (hasError) {
    alert(errorMsg);
    return;
  }
...
// 1️⃣ When user changes the main release date (Step 1)
$('#release_date').on('change', function () {
  applyOutletReleaseDates();
});

// 2️⃣ When user navigates to Step 5 (Outlets)
