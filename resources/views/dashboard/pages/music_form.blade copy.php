...
// Remove a single audio before upload
/*$('#audioList').on('click', '.remove-audio', function() {
  const item = $(this).closest('.audio-item');
  const filename = item.find('strong').text();
  item.remove();

  // Remove from uploadedFilesMeta array
  uploadedFilesMeta = uploadedFilesMeta.filter(f => f.filename !== filename);

  // If no more audios, reset input
  if (uploadedFilesMeta.length === 0) {
    $('#audios').val('');
    $('#audioList').empty();
  }
...
// Clear all audios
/*$('#audioList').on('click', '#clearAllAudios', function() {
  if (confirm('Remove all selected audio files?')) {
    uploadedFilesMeta = [];
    $('#audios').val('');
    $('#audioList').empty();
  }
