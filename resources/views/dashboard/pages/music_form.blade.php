    ...
    success(resp) {
      if (resp.status === 'processing') {
        ...
        $('#audioUploadStatus').append(
          '<div class="text-muted mt-2">Processing audio files... please wait</div>'
        );
        ...
        pollAudioUploadStatus(resp.cache_key, (files, releaseId) => {
          ...
          // --- MERGE newly uploaded tracks into global track list ---
          files.forEach(file => {
            const exists = window.__allTracks.find(t => t.filename === file.filename);
            if (!exists) window.__allTracks.push(file);
            else Object.assign(exists, file); // update if server returned changes
          });

          // --- Rebuild the entire track UI from authoritative array ---
          rebuildTrackUI();
    ...
    window.__allTracks.forEach(track => {
      ...
      $audioList.append(`
        <div class="mb-3 p-2 border rounded bg-light position-relative audio-item" data-track-id="${track.track_id || ''}">
          ...
          <audio controls class="mt-2 w-100"><source src="${track.audio_url}" type="audio/mpeg"></audio>
        </div>
      `);
